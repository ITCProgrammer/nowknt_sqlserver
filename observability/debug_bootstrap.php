...
if (!function_exists('qcf_debug_sql_identifier')) {
    function qcf_debug_sql_identifier($value, $default) {
...
if (!function_exists('qcf_debug_monitor_parts')) {
    function qcf_debug_monitor_parts() {
        static $parts = null;
        if (is_array($parts)) {
            return $parts;
        }
        ...
        $config = qcf_debug_config();
        $parts = array(
            'database' => qcf_debug_sql_identifier(isset($config['monitor_database']) ? $config['monitor_database'] : '', 'dbknitt'),
            'schema' => qcf_debug_sql_identifier(isset($config['monitor_schema']) ? $config['monitor_schema'] : '', 'observability'),
            'table' => qcf_debug_sql_identifier(isset($config['monitor_table']) ? $config['monitor_table'] : '', 'tbl_debug_monitor'),
        );
...
if (!function_exists('qcf_debug_monitor_table_sql')) {
    function qcf_debug_monitor_table_sql() {
        $parts = qcf_debug_monitor_parts();
        return '[' . $parts['database'] . '].[' . $parts['schema'] . '].[' . $parts['table'] . ']';
    }
}

if (!function_exists('qcf_debug_monitor_table_literal')) {
    function qcf_debug_monitor_table_literal() {
        $parts = qcf_debug_monitor_parts();
        return $parts['database'] . '.' . $parts['schema'] . '.' . $parts['table'];
    }
}
...
if (!function_exists('qcf_debug_upgrade_table_schema')) {
    function qcf_debug_upgrade_table_schema($conn) {
        ...
        $tableSql = qcf_debug_monitor_table_sql();
        $tableLiteral = qcf_debug_monitor_table_literal();

        $upgradeSql = "
IF COL_LENGTH('{$tableLiteral}', 'error_trace') IS NULL
BEGIN
    ALTER TABLE {$tableSql} ADD error_trace NVARCHAR(MAX) NULL;
END
";
...
if (!function_exists('qcf_debug_table_ready')) {
    function qcf_debug_table_ready() {
        ...
        $checkSql = "SELECT 1
                     FROM [{$parts['database']}].sys.tables t
                     INNER JOIN [{$parts['database']}].sys.schemas s ON s.schema_id = t.schema_id
                     WHERE t.name = ? AND s.name = ?";
        $checkParams = array($parts['table'], $parts['schema']);
        $stmt = @sqlsrv_query($conn, $checkSql, $checkParams);
        if ($stmt !== false) {
        ...
        $createSql = "
IF NOT EXISTS (SELECT 1 FROM [{$parts['database']}].sys.schemas WHERE name = '{$parts['schema']}')
BEGIN
    EXEC('CREATE SCHEMA [{$parts['schema']}]');
END

IF OBJECT_ID('{$tableLiteral}', 'U') IS NULL
BEGIN
    ...
    CREATE INDEX IX_tbl_debug_monitor_created_at ON {$tableSql}(created_at DESC);
    CREATE INDEX IX_tbl_debug_monitor_log_type ON {$tableSql}(log_type);
END
";
        @sqlsrv_query($conn, $createSql);

        $stmt2 = @sqlsrv_query($conn, $checkSql, $checkParams);
        if ($stmt2 !== false) {
...
if (!function_exists('qcf_debug_build_record')) {
    function qcf_debug_build_record($fatalError = null) {
        ...
        $userName = qcf_debug_safe_text(qcf_debug_session_first(array('usrid', 'user_id10', 'username', 'user', 'user_name')), 100);
        $sessionUserRaw = qcf_debug_session_first(array('user_id', 'id10', 'iduser'));
