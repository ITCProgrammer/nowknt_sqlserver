...
ORDER BY PROGRESSSTATUS ASC
 ";
	 ...
	 $hslkg=$rowdb2['INSKG']+$rowdb22['Q3KG']+$rowdb22['KGSISA']+$rowd['berat_awal']+$rowd1['berat_awal']+$rowd2['berat_awal'];
	 ...
	 $TprsnLoss=0; 
      ...
      <td><?php echo round($rowdb22['Q3KG'],2); ?></td>
      <td><?php echo $prsn3; ?></td>
...
function cetak_cek() {
	...
	var project = document.querySelector('input[name="no_project"]').value;

    if (awal != "" && akhir != "" && status != "") {
      let url = 'pages/cetak/cetak-benang-demand-new.php?awal=' + awal + '&akhir=' + akhir + '&status=' + status + '&project=' + project + '';
