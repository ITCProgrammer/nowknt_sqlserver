...
include("../koneksi.php");
	...
	$TrnNO=substr($modal_id,0,12);	
	$ORDERCODE=substr($modal_id,13,8);	
	$TGL=substr($modal_id,22,10);
	$pos=substr($modal_id,33,300);
	$poss=strpos($pos,"*");	
	$UID=substr($modal_id,33,$poss);
	$pos1=substr($modal_id,(34+$poss),200);
	$poss1=strpos($pos1,"*");
	$LOTCODE=substr($modal_id,(34+$poss),$poss1);
...
?>
		...
		AND STOCKTRANSACTION.LOTCODE ='$LOTCODE' AND STOCKTRANSACTION.TRANSACTIONDATE='$TGL' AND STOCKTRANSACTION.CREATIONUSER='$UID' 
		AND STOCKTRANSACTION.TRANSACTIONNUMBER='$TrnNO' AND 	
