...
include("../koneksi.php");
	...
	$dt1=substr($modal_id,20,400);
	$pos1=strpos($dt1,"*");
	$UID=substr($dt1,0,$pos1);
	$dt2=substr($dt1,$pos1+1,300);
	$pos2=strpos($dt2,"*");	
	$LOTCODE=substr($dt2,0,$pos2);
	$KD=substr($dt2,($pos2+1),200);
...
?>
			...
			<table id="lookup1" class="table table-sm table-bordered table-hover table-striped" width="100%" style="font-size: 14px;">
						<thead>
							<tr>
								...
								<th><div align="center">QUALITY</div></th>
		...
		WHERE STOCKTRANSACTION.RETURNTRANSACTION ='1' AND (STOCKTRANSACTION.LOGICALWAREHOUSECODE='P501' OR STOCKTRANSACTION.LOGICALWAREHOUSECODE='M501' OR STOCKTRANSACTION.LOGICALWAREHOUSECODE='M904') AND STOCKTRANSACTION.ORDERCODE='$ORDERCODE'
		AND STOCKTRANSACTION.LOTCODE ='$LOTCODE' AND STOCKTRANSACTION.TRANSACTIONDATE='$TGL' AND STOCKTRANSACTION.CREATIONUSER='$UID' AND 		
CONCAT(TRIM(DECOSUBCODE01),
CONCAT(' ',CONCAT(TRIM(DECOSUBCODE02),
CONCAT(' ',CONCAT(TRIM(DECOSUBCODE03),
CONCAT(' ',CONCAT(TRIM(DECOSUBCODE04),
CONCAT(' ',CONCAT(TRIM(DECOSUBCODE05),
CONCAT(' ',CONCAT(TRIM(DECOSUBCODE06),
CONCAT(' ',CONCAT(TRIM(DECOSUBCODE07),
CONCAT(' ',TRIM(DECOSUBCODE08)))))))))))))))='$KD'
		";					  
	...
	<td align=center>$rD[QUALITYLEVELCODE]</td>
