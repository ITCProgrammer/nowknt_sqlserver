...
$sqlDB2 =" 
SELECT ADSTORAGE.VALUESTRING,AD1.VALUEDATE, ITXVIEWKNTORDER.*,CURRENT_TIMESTAMP AS TGLS FROM ITXVIEWKNTORDER 
...
trim(LISTAGG (PROGRESSSTATUS , ',') WITHIN GROUP(ORDER BY PRODUCTIONDEMANDCODE ASC)) as IDS	
FROM PRODUCTIONDEMANDSTEP 
WHERE PRODUCTIONDEMANDCODE ='$rowdb2[PRODUCTIONDEMANDCODE]' AND 
(OPERATIONCODE='INS1' OR OPERATIONCODE='KNT1' OR OPERATIONCODE='PBS' OR OPERATIONCODE='AMC')
GROUP BY PRODUCTIONDEMANDCODE ";	
...
$rowdb29 = db2_fetch_assoc($stmt9);	
		...
		elseif($rowdb2['PROGRESSSTATUS']=="2" and $rowdb29['IDS']=="2 ,2 ,0 ,2" ){
			$stts="<small class='badge badge-warning'><i class='far fa-clock text-white blink_me'></i> Antri Mesin</small>";
			$totHari=abs($hariPC);
