...
$rowdb2IN = db2_fetch_assoc($stmtIN);
	 ...
	 $prdqt=$rowd['berat_awal']+$rowd1['berat_awal']+$rowd2['berat_awal'];
	 if($prdqt>0){
		$prdqty1=$rowd['berat_awal']+$rowd1['berat_awal']+$rowd2['berat_awal'];
	 }else{
		$prdqty1=0;
	 }
	 $prdqty+=$prdqty1;
...
//	 $prsnLoss1=0; 	 
//	 }	
	 $kgpakai = floatval($rowdb21L['KGPAKAI']);
	 $kgsisa = floatval($rowdb22L['KGSISA']);
	 $inskgs = floatval($rowdb2IN['INSKG']);
	 $denominator = $kgpakai - $kgsisa;

	 if ($kgpakai > 0 && $kgsisa > 0 && $inskgs > 0 && $denominator != 0) {
		$prsnLoss = round(($losskg / $denominator) * 100, 2);
		$prsnLoss1 = round(($losskg12 / $denominator) * 100, 2);
	 } else {
		$prsnLoss = 0;
		$prsnLoss1 = 0;
	 }
	 ...
	 $prsnLoss1T+=$prsnLoss1;
      ...
      <td style="text-align: right"><?php //echo round($losskg1T,3);
		  echo round($losskg1,3);?></td>
      <td style="text-align: right"><?php echo round($prsnLoss1T,3); ?></td>
      </tr>	  				  
	<?php 
	 $no++;
 		$tper+=$persen;
