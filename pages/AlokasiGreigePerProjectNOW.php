...
$sqlDB1S = " SELECT SUM(p2.USEDUSERPRIMARYQUANTITY) AS USEDUSERPRIMARYQUANTITY  FROM PRODUCTIONDEMAND p 
LEFT OUTER JOIN PRODUCTIONRESERVATION p2 ON p2.ORDERCODE = p.CODE AND p2.ITEMTYPEAFICODE ='KGF'
LEFT OUTER JOIN ADSTORAGE a  ON p.ABSUNIQUEID = a.UNIQUEID  AND a.NAMENAME  ='SplittedFrom' 
WHERE a.VALUESTRING ='".$rowdb['CODE']."' ";
$stm1S  = db2_exec($conn1,$sqlDB1S, array('cursor'=>DB2_SCROLLABLE));	 
$rowdb1S = db2_fetch_assoc($stm1S);
	 $BagiQTY = $rowdb1S['USEDUSERPRIMARYQUANTITY']+$rowdb['USEDBASEPRIMARYQUANTITY'];
	 ...
	 $tOrder+=$qtyprOrder;
      ...
      <td style="text-align: right"><?php echo number_format(round($BagiQTY,2),2); ?></td>
