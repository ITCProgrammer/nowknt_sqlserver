...
$c=0;
	...
	SUM(BASESECONDARYQUANTITYUNIT) AS QTY_CONES,WHSLOCATIONWAREHOUSEZONECODE,WAREHOUSELOCATIONCODE,LOGICALWAREHOUSECODE,ELEMENTSCODE,BALANCE.ITEMTYPECODE
		...
		WHERE $Wknt $WZone $Wlot $WLokasi $WCode1 $WCode2 $WCode3 $WCode4 $WCode5 $WCode6 $WCode7 $WCode8 (BALANCE.ITEMTYPECODE='GYR' OR BALANCE.ITEMTYPECODE='DYR') AND ( NOT BALANCE.WAREHOUSELOCATIONCODE='Z00' AND NOT BALANCE.WAREHOUSELOCATIONCODE='Z99')
		GROUP BY 
...
$sqlDB23 = " SELECT SUPPLIERCODE  FROM LOT WHERE CODE='$lot' AND NOT SUPPLIERCODE IS NULL ";
...
if($posLot>0){
	...
	$lotP=$lot;
      ...
      <td><?php if($rowdb24['TG']!=""){echo date('d-m-Y', strtotime($rowdb24['TG']));} ?></td>
      <td><?php echo $rowdb24['SP']." - ".$rowdb24['NT']; ?></td>
      </tr>	  				  
	<?php 
	 $no++; 
