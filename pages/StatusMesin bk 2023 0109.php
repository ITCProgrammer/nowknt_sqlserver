		...
		<div class="card card-success">
              ...
              <div class="card-body table-responsive">
                <table id="example12" width="100%" class="table table-sm table-striped table-bordered table-hover" style="font-size: 13px;">
                  ...
                  <tbody>
				  <?php
		$sql=mysqli_query($con," SELECT no_mesin,kd_dtex FROM tbl_mesin WHERE not isnull(kd_dtex) ORDER BY no_mesin ASC ");
					  
   $no=1;   
   $c=0;
    while($rowd=mysqli_fetch_array($sql)){
$totHari="";
$sql1 = " SELECT CODE,LONGDESCRIPTION, SHORTDESCRIPTION FROM DB2ADMIN.USERGENERICGROUP 
		WHERE USERGENERICGROUPTYPECODE = 'MCK' AND 
		USERGENGROUPTYPECOMPANYCODE = '100' AND 
		OWNINGCOMPANYCODE = '100' AND
		CODE ='$rowd[kd_dtex]'
		";
$stm	= db2_exec($conn1,$sql1, array('cursor'=>DB2_SCROLLABLE));
$rMCK	= db2_fetch_assoc($stm);		
$sqlDB2 =" 
...
$rowdb29 = db2_fetch_assoc($stmt9);	
		...
		$StHari=round($rowdb2['BASEPRIMARYQUANTITY']/round($rowdb23['STDRAJUT'],0));	
		if($kHari>=0){
			if($StsJ=="ya"){
	...
	if($rBS['kg_bs']>0 and $rowdb22['JQTY']>0){
      ...
      <td style="text-align: center"><?php echo $rMCK['SHORTDESCRIPTION'];?></td>
      <td><?php echo $rMCK['LONGDESCRIPTION'];?></td>
	  ...
	  <td style="text-align: right"><?php echo round($rowdb2['BASEPRIMARYQUANTITY'],2);?></td>	  
      ...
      <td style="text-align: center"><?php if($rowdb2['FINALSCHEDULEDACTUALDATE']!="" and $rowdb2['PRODUCTIONDEMANDCODE']!=""){echo $rowdb2['FINALSCHEDULEDACTUALDATE'];}else{ if($tglRFin=="1970-01-01"){}else{echo $tglRFin;}}?></td>
	...
	<?php 
		$stmtCK   = db2_exec($conn1,$sqlDB2, array('cursor'=>DB2_SCROLLABLE)); 
		$rdb2CK = db2_fetch_assoc($stmtCK);
		if($rdb2CK['PRODUCTIONDEMANDCODE']==""){
					  ?>				  
	<tr>
	    <td style="text-align: center"><?php echo $rowd['no_mesin']; ?></td>
	    <td style="text-align: center"><?php echo $rowd['kd_dtex']; ?></td>
	    ...
	    <td style="text-align: right">&nbsp;</td>
	    <td style="text-align: center">&nbsp;</td>
	    <td style="text-align: center"><?php echo "Tidak Ada PO";?></td>
	    <td style="text-align: center">&nbsp;</td>
	    <td style="text-align: center">&nbsp;</td>
	    <td style="text-align: right">&nbsp;</td>
	    <td style="text-align: center">&nbsp;</td>
