		...
		<div class="card-body">
			<table id="example1" class="table table-sm table-bordered table-striped" style="font-size:13px;">
				...
				<tbody>
					...
					$sqlText = "SELECT
									FORMAT(a.tgl_produksi, 'dd/MM/yy') AS tgl,
									a.id,
									a.no_mesin,
									a.nama,
									a.leader,
									...
									(
										CAST(SUM(b.counter) AS decimal(18,4)) / NULLIF(CAST(a.count_roll AS decimal(18,4)),0)
									) AS roll,
									c.total,
									(
										a.std_kg * (
											CAST(SUM(b.counter) + a.counter_akhir - a.counter_awal AS decimal(18,4)) / NULLIF(CAST(a.count_roll AS decimal(18,4)),0)
										)
									) AS berat,
									(
										(a.std_kg / NULLIF(CAST(a.count_roll AS decimal(18,4)),0)) * ABS(SUM(b.counter) - a.t_count)
									) AS selisih_kg,
									(
										CASE
										WHEN a.grade = 'A' THEN
									...
									(
										(
											(
												(
													(
														CAST(SUM(b.counter) + a.counter_akhir - a.counter_awal AS decimal(18,4))
													) / NULLIF(CAST(a.t_count AS decimal(18,4)),0)
												)
											) * 100
										) / (
											CASE
									...
									a.tgl_buat
								FROM
									dbknitt.tbl_rajut_produksi_now a
								LEFT JOIN dbknitt.tbl_rajut_produksi_detail_now b ON a.id = b.id_rajut
								LEFT JOIN dbknitt.tbl_mesin_stop_now c ON a.id = c.id_rajut
								WHERE  $Tgl $Shft 
								GROUP BY
									a.id,
									a.no_mesin,
					...
					?>
						<tr>
							...
							<td><?php // echo number_format(round($rowd[eff],'2'),'2'); 
								?><?php echo $eff; ?></td>
							<td><?php // echo number_format(round($rowd[capai],'2'),'2'); 
								?><?php echo $hsl; ?></td>
