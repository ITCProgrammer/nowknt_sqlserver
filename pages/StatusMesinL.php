...
WHERE ITXVIEWKNTORDER.ITEMTYPEAFICODE ='KGF' AND (PRODUCTIONDEMAND.PROGRESSSTATUS='2' OR AD6.VALUESTRING='1') 
...
if($rowdb2['PROJECTCODE']!=""){$proj=$rowdb2['PROJECTCODE'];}else{$proj=$rowdb2['ORIGDLVSALORDLINESALORDERCODE'];}	
...
$rowdb29 = db2_fetch_assoc($stmt9);	
	...
	if($rowdb2['BASEPRIMARYQUANTITY']>0){
	  ...
	  <td style="text-align: right"><?php if(substr($rowdb2['PROJECTCODE'],0,3)=="OPN" or substr($rowdb2['ORIGDLVSALORDLINESALORDERCODE'],0,3)=="OPN"){echo "<a href='#' class='show_detail_allokasi' id='".$proj."'>".round($rKBG['tot'])-round($rowdb2['JQTY_J2'],2)."</a>";}else{echo (round(($rowdb2['BASEPRIMARYQUANTITY']+$rowdb2['QTYOPOUT'])-($rowdb2['QTYSALIN']+$rowdb2['QTYOPIN']),2))-round($rowdb2['JQTY_J2'],2);}?></td>	  
