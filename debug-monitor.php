...
function qcf_dm_sql_identifier($value, $default) {
...
function qcf_dm_monitor_parts() {
    ...
    $parts = array(
        'database' => qcf_dm_sql_identifier(isset($config['monitor_database']) ? $config['monitor_database'] : '', 'dbknitt'),
        'schema' => qcf_dm_sql_identifier(isset($config['monitor_schema']) ? $config['monitor_schema'] : '', 'observability'),
        'table' => qcf_dm_sql_identifier(isset($config['monitor_table']) ? $config['monitor_table'] : '', 'tbl_debug_monitor'),
    );

    return $parts;
}

function qcf_dm_monitor_table_sql() {
    $parts = qcf_dm_monitor_parts();
    return '[' . $parts['database'] . '].[' . $parts['schema'] . '].[' . $parts['table'] . ']';
}

function qcf_dm_monitor_table_literal() {
    $parts = qcf_dm_monitor_parts();
    return $parts['database'] . '.' . $parts['schema'] . '.' . $parts['table'];
}
...
function qcf_dm_has_column($conn, $columnName) {
    ...
    $tableLiteral = qcf_dm_monitor_table_literal();
    $sql = "SELECT COL_LENGTH('{$tableLiteral}', '{$columnName}') AS col_len";
...
function qcf_dm_access_state($config) {
    if (!empty($config['allow_public_access'])) {
        return 'ok';
    }

    $sessionUser = qcf_dm_session_value(array('usrid', 'user_id10', 'username', 'user', 'user_name', 'iduser'));
    if ($sessionUser === null) {
        return 'need_login';
    }

    // Legacy lockscreen flow (old apps): only enforce if legacy auth keys are used.
    $legacyUser = qcf_dm_session_value(array('usrid', 'user_id10'));
    if ($legacyUser !== null) {
    ...
    $currentRole = (string)qcf_dm_session_value(array('lvl_id', 'lvl_id10', 'option', 'role'), '');
...
if ($accessState === 'need_login') {
    $loginUrl = isset($config['login_url']) ? trim((string)$config['login_url']) : '';
    if ($loginUrl === '' && is_file(__DIR__ . DIRECTORY_SEPARATOR . 'login.php')) {
        $loginUrl = 'login';
    }
    if ($loginUrl !== '') {
...
if ($accessState === 'need_unlock') {
    $lockscreenUrl = isset($config['lockscreen_url']) ? trim((string)$config['lockscreen_url']) : '';
    if ($lockscreenUrl === '' && is_file(__DIR__ . DIRECTORY_SEPARATOR . 'lockscreen.php')) {
        $lockscreenUrl = 'lockscreen';
    }
    if ($lockscreenUrl !== '') {
    ...
    echo "Access denied: session needs unlock";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['dm_action']) && $_POST['dm_action'] === 'truncate') {
    ...
    if ($tableReadyNow && (is_resource($dbConnNow) || is_object($dbConnNow))) {
        $tableSql = qcf_dm_monitor_table_sql();
        if ($mode === 'all') {
            $sqlDelete = "DELETE FROM {$tableSql}";
            $stmtDelete = @sqlsrv_query($dbConnNow, $sqlDelete);
        } else {
            $sqlDelete = "DELETE FROM {$tableSql} WHERE created_at < DATEADD(DAY, -?, SYSDATETIME())";
...
<head>
    <meta charset="utf-8">
    <title>Laporan SQLServer Debug Monitor</title>
...
<div class="wrap">
    <div class="panel-card">
        ...
        <div class="row">
            <div class="col-sm-7">
                <h3 style="margin-top:0;margin-bottom:8px;">Laporan SQLServer Debug Monitor</h3>
                <div class="toolbar">
